...
<script type="text/javascript">
	function next(){
		 var ok_01 = document.getElementById("ok_01");
		 var ok_02 = document.getElementById("ok_02");
		 
		 var result = false;
		 
		 if(ok_01.checked && ok_02.checked){
			result = true; 
		 }else{
			result = false;
		 }
		 if(result){
			 $.ajax({url:"<%=application.getContextPath()%>/mem/join2.jsp",
					type:"POST",
			 ...
			 <%-- location.href = "<%=application.getContextPath()%>/mem/join2.jsp"; --%>
		 }else{
			 alert("약관에 동의 하십시오.");
		 }
	}
	/* $(function(){
		...
		$("#modal_04_").html('<img src="/semiProject/image/hr/join.jpg">');
...
<style type="text/css">
	...
	#button_01 {
		width: 155px;
		height: 58px;
		background-image: url("/semiProject/image/hr/join_01_button.png");
		border: none;
		margin-left: 207.5px;
		cursor: pointer;
	}
	#button_02 {
		...
		height: 58px;
		border: none;
		background-image: url("/semiProject/image/hr/btn_agree_cancel.jpg");
		margin-left: 20px;
		cursor: pointer;
	}
</style>
	...
	<div id="join_01">
		<div id="join_02">
			<div>
				<img src="/semiProject/image/hr/memberImage_04.jpg">
			</div>
			<hr>
			<div id="join_02_01">
				<img src="/semiProject/image/hr/memberImage_01.jpg">
			</div>
			<div id="join_02_02">
				회원가입을 원하실 경우, 아래의 서비스 약관과 개인정보수집 및 이용에 대한 안내를 반드시 읽고 동의해 주세요.<br>
				동의를 원치 않으실 경우 회원가입이 제한됩니다.
			</div>
			<div id="join_02_03">
				<div id="join_02_03_01">
				<br>
				<br>
					<div class="text">
					<img src="/semiProject/image/hr/memberImage_02.png">
					<br>
						<textarea id="textarea1" readonly="readonly">
제1조(목적)
   
이 약관은 브로큰윈도우 회사(전자거래 사업자)가 운영하는 [쇼핑몰명] 사이버 몰(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다. 
※ 「PC통신등을 이용하는 전자거래에 대해서도 그 성질에 반하지 않는한 이 약관을 준용합니다」 
 
 
제2조(정의)
   
① "몰"이란 브로큰윈도우 회사가 재화 또는 용역을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화 또는 용역을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.  
...
부칙
1. 이 약관은 2015년 07월 01일부터 적용됩니다.
							</textarea>
						</div>
						<div id="h_1">
							이용약관에 동의하십니까?
							<input type="radio" name="check" id="ok_01"> 동의합니다.					
							<input type="radio" name="check"> 동의하지 않습니다.
						</div>
					</div>
					<div id="join_02_03_02">
						<div class="text">
						<img src="/semiProject/image/hr/memberImage_03.png">
						<br>
							<textarea id="textarea1" readonly="readonly">
브로큰윈도우 개인정보취급방침

브로큰윈도우(이하 “회사”라 함)는 이용자들의 개인정보보호를 매우 중요시하며,  "정보통신망 이용촉진 및 정보보호에 관한 법률”을 준수하고 있습니다.
회사는 개인정보취급방침을 통하여 고객님께서 제공하시는 개인정보가 어떠한 용도와 방식으로 
이용되고 있으며, 개인정보보호를 위해 어떠한 조치가 취해지고 있는지 알려드립니다. 
회사는 개인정보취급방침을 개정하는 경우 웹사이트 공지사항(또는 개별공지)을 통하여 공지할 것입니다.
 
브로큰윈도우의 개인정보 취급방침은 다음과 같은 내용을 담고 있습니다.
...
단, 상법 및 “전자상거래등에서의 소비자보호에 관한 법률” 등 관련 법령의 규정에 의하여 다음과 같이 거래 
관련 관리 의무 관계의 확인 등을 이유로 일정기간 보유하여야 할 필요가 있을 경우에는 일정기간 보유합니다.
...
5. 개인정보의 제3자 제공 
회사는 고객님의 개인정보를 회원가입 시 아래와 같이 고지 또는 명시하여 동의 받은 범위 내에서 
제3자에게 제공합니다.

제공받는자 : KG 이니시스 
제공항목 : 주문자 정보
제공 목적 : 전자상거래를 위한 전자결제용
보유 및 이용기간 : 3년


이용자의 개인정보는 원칙적으로 위 사항 외에 외부에 제공하지 않습니다. 
다만, 아래의 경우에는 예외로 합니다.
- 이용자들이 사전에 동의한 경우
정보수집 또는 정보제공 이전에 회원님께 비즈니스 파트너가 누구인지, 어떤 정보가 왜 필요한지, 
그리고 언제까지 어떻게 보호?관리되는지 알려드리고 동의를 구하는 절차를 거치게 되며, 
회원님께서 동의하지 않는 경우에는 추가적인 정보를 수집하거나 비즈니스 파트너와 공유하지 않습니다.
- 법령의 규정에 의거하거나, 수사 목적으로 법령에 정해진 절차와 방법에 따라 수사기관의 요구가 있는 경우

6. 개인정보의 취급위탁
원활한 업무 처리를 위해 이용자의 개인정보를 위탁 처리할 경우 반드시 사전에 개인정보 
처리위탁을 받는 자(이하 ‘수탁자’라 합니다)와 개인정보 처리위탁을 하는 업무의 내용을 고지합니다. 
현재 회사의 개인정보처리 수탁자와 그 업무의 내용은 다음과 같습니다.

위탁 대상자(수탁자) : 위탁업무 내용(수탁업무)
[배송업체명 기입]   : 상품 배송 업무 및 배송위치 / 도착정보 등의 서비스 제공
서울신용평가정보㈜ : 본인인증
[PG업체명 기입]    :  결제관련

7. 이용자 및 법정대리인의 권리와 그 행사방법

가. 이용자 및 법정 대리인은 언제든지 등록되어 있는 자신 혹은 당해 만 14세 미만 아동의 
개인정보를 조회하거나 수정할 수 있으며 가입해지(동의철회)를 요청할 수도 있습니다.
나. 이용자 혹은 만 14세 미만 아동의 개인정보 조회, 수정을 위해서는 
“개인정보변경”(또는 “회원정보수정” 등)을, 가입해지(동의철회)를 위해서는 "회원탈퇴"를 
클릭하여 본인 확인 절차를 거치신 후 직접 열람, 정정 또는 탈퇴가 가능합니다.
다. 혹은 개인정보관리책임자에게 서면, 전화 또는 이메일로 연락하시면 지체 없이 조치하겠습니다.
라. 이용자가 개인정보의 오류에 대한 정정을 요청하신 경우에는 정정을 완료하기 전까지 
...
8. 쿠키(cookie)의 운영에 관한 사항
회사는 이용자(접속자)의 정보를 수시로 저장하고 찾아내는 '쿠키(cookie)' 등을 운용합니다. 
쿠키란 웹사이트를 운영하는데 이용되는 서버가 귀하의 브라우저에 보내는 아주 작은 텍스트 파일로서 
귀하의 컴퓨터 하드디스크에 저장됩니다. 회사는 다음과 같은 목적을 위해 쿠키를 사용합니다.

가. 쿠키 등 사용 목적
회원과 비회원의 접속빈도나 방문시간 등을 분석, 이용자의 취향과 관심분야를 파악 및 자취 추적, 
각종 이벤트 참여 정도 및 방문 회수 파악 등을 통한 타겟마케팅 및 개인 맞춤 서비스 제공
귀하는 쿠키 설치에 대한 선택권을 가지고 있습니다. 따라서, 귀하는 웹 브라우저에서 
옵션을 설정함으로써 모든 쿠키를 허용하거나, 쿠키가 저장될 때마다 확인을 거치거나, 
아니면 모든 쿠키의 저장을 거부할 수도 있습니다.

나. 쿠키 설정 거부 방법
이용자는 사용하는 웹 브라우저의 옵션을 선택함으로써 모든 쿠키를 허용하거나, 쿠키를 저장할 
때마다 확인을 거치거나, 모든 쿠키의 저장을 거부하도록 선택하여 설정할 수 있습니다.
...
가. 귀하께서는 회사의 서비스를 이용하시며 발생하는 모든 개인정보보호 관련 민원을 개인정보관리책임자 혹은 담당부서로 신고하실 수 있습니다.
...
10. 고지의 의무
현 개인정보취급방침은 2012년 00월 00일부터 적용됩니다. 내용의 추가, 삭제 및 수정이 
있을 시에는 개정 최소 7일전부터 홈페이지의 공지사항을 통하여 고지할 것입니다. 
...
본 방침은 : 2015년 07월 01일부터 시행됩니다.
						...
						<div id="h_2">
							이용약관에 동의합니까?
							<input type="radio" name="check_02" id="ok_02"> 동의합니다.
							<input type="radio" name="check_02"> 동의하지 않습니다.
						</div>
						<br>
						<hr size="1" color="#d9d9d9">
					</div>
						<div id="h_3">회원가입시 기재하신 개인정보는 서비스 이행을 위한 자료로써만 활용되며, 기타 위탁업체 및 타업체로서 정보제공이 되지 않음을 알려드립니다.
						</div>
				</div>
				<div id="button">
					<input type="button" id="button_01" onclick="next()">
					<input type="button" id="button_02" onclick="cancle()">
				</div>
		</div>
	</div>
</div>